...
async function uploadIDDocuments(page: Page) {
  // 上傳身分證正面
  await uploadFile(page, '#IDFRONT', 'test-files/id-01.jpg', true);

  // 上傳身分證背面
  await uploadFile(page, '#IDBACK', 'test-files/id-02.jpg');

  // 上傳第二證件正面
  await uploadFile(page, '#OTHERFRONT', 'test-files/id-03.png');
}

async function uploadSelfieWithID(page: Page) {
  await uploadFile(page, '#SELFIE_WITH_ID', 'test-files/id-04.jpg');
}
...
async function fillPersonalInfo(page: Page) {
  // 基本資料
  await page.getByPlaceholder('請輸入戶名').fill('測試部二十一');
